-- FINAL COLLABORATOR REPAIR — safe to re-run.
-- Run this once in the production Supabase SQL Editor after the collaborator feature deployment.

-- 1) Ensure the collaborator tables exist.
...
-- The referral code is the unique identifier; display names are labels.
alter table public.collaborator_links drop constraint if exists collaborator_links_user_id_name_key;
...
create unique index if not exists collaborator_partner_products_uidx on public.collaborator_partner_products(user_id, product_id);
create index if not exists collaborator_partner_products_user_idx on public.collaborator_partner_products(user_id);

-- 2) Product-aware referral page views and order attribution.
...
-- 3) Keep server-side and Supabase RLS admin identity consistent.
...
create policy "admins manage collaborator partner products" on public.collaborator_partner_products
for all to authenticated using (public.is_admin()) with check (public.is_admin());

drop policy if exists "collaborators read own product access" on public.collaborator_partner_products;
create policy "collaborators read own product access" on public.collaborator_partner_products
for select to authenticated using (auth.uid() = user_id or public.is_admin());

drop policy if exists "public create page views" on public.page_views;
create policy "public create page views" on public.page_views
for insert to anon, authenticated
with check (
  collaborator_code is null
  or exists (
    select 1 from public.collaborator_links cl
    where cl.id = collaborator_link_id and cl.code = collaborator_code and cl.active = true
  )
);
...
-- 4) Make legacy plain INSERT clients idempotent for product access.
create or replace function public.ignore_duplicate_collaborator_product_access()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
  if exists (
    select 1 from public.collaborator_partner_products
