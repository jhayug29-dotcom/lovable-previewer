...
create table if not exists public.profiles (
  ...
  email text, full_name text, avatar_url text, created_at timestamptz default now()
);
create table if not exists public.user_roles (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  role public.app_role not null, created_at timestamptz default now(), unique (user_id, role)
);
...
create policy "read own roles" on public.user_roles for select to authenticated using (auth.uid() = user_id or public.is_admin());
...
create table if not exists public.products (
  id uuid primary key default gen_random_uuid(), slug text unique not null, title text not null, tagline text default '', description text default '',
  category text not null default 'After Effects', cover_url text, banner_url text, video_url text, price numeric not null default 0,
  original_price numeric not null default 0, is_free boolean not null default false, badge text, features text[] not null default '{}',
  file_info text[] not null default '{}', how_to_use jsonb not null default '[]'::jsonb, download_link text, rating numeric not null default 5,
  sales integer not null default 0, active boolean not null default true, show_on_homepage boolean not null default true,
...
create table if not exists public.product_sections (
  ...
  title text not null, content text not null default '', sort_order integer not null default 0, enabled boolean not null default true,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists public.reviews (
  id uuid primary key default gen_random_uuid(), product_id uuid not null references public.products(id) on delete cascade,
  name text not null, handle text default '', rating integer not null default 5 check (rating between 1 and 5), body text not null, created_at timestamptz default now()
);
create table if not exists public.coupons (
  id uuid primary key default gen_random_uuid(), code text unique not null, percent_off integer not null check (percent_off between 1 and 100), active boolean not null default true,
  expires_at timestamptz, max_uses integer, used_count integer not null default 0, created_at timestamptz default now(), product_ids uuid[] not null default '{}'
);
create table if not exists public.banners (
  id uuid primary key default gen_random_uuid(), title text not null, subtitle text default '', image_url text, link_url text,
  active boolean not null default true, sort_order integer not null default 0, created_at timestamptz default now()
);
...
create table if not exists public.orders (
  id uuid primary key default gen_random_uuid(), user_id uuid references auth.users(id) on delete set null, product_id uuid references public.products(id) on delete set null,
  cf_order_id text unique, amount numeric not null default 0, currency text not null default 'INR', status text not null default 'PENDING', coupon_code text,
  customer_email text, customer_name text, customer_phone text, download_link text, origin text, paid_at timestamptz, receipt_sent_at timestamptz, created_at timestamptz default now()
);
...
create table if not exists public.site_settings (
  id text primary key default 'global', support_email text not null default '', contact_email text not null default '', phone text not null default '',
  whatsapp text not null default '', address text not null default '', support_hours text not null default '', instagram text not null default '', youtube text not null default '',
  twitter text not null default '', refund_policy text not null default '', licence_note text not null default '', updated_at timestamptz not null default now()
);

-- Supabase 2026 Data API deployments require explicit grants.
...
select u.id, 'admin'::public.app_role from auth.users u where lower(u.email) = lower('growchannel2026@gmail.com') on conflict (user_id, role) do nothing;
insert into public.profiles (id, email, full_name)
select u.id, u.email, u.raw_user_meta_data->>'full_name' from auth.users u on conflict (id) do nothing;
insert into public.site_settings (id, support_email, contact_email, support_hours)
values ('global', 'growchannel2026@gmail.com', 'growchannel2026@gmail.com', 'Mon–Sat, 10:00–19:00 IST') on conflict (id) do nothing;
