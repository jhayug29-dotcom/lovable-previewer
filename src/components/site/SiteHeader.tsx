      ...
      <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-4 sm:px-6 sm:py-5 lg:px-12">
        <Link to="/" className="group flex items-center gap-2.5 sm:gap-3">
          <span className="glass flex size-10 items-center justify-center rounded-2xl transition-transform duration-500 group-hover:scale-105">
            <img src="/editly-icon-iris.svg" alt="Editly Store logo" className="site-logo-mark size-7 object-contain" width={28} height={28} />
          </span>
          <div className="flex flex-col">
            <span className="font-display text-[1.15rem] font-extrabold tracking-tight text-ink sm:text-[1.35rem]">Editly Store</span>
            {isIndependenceMode && <span className="animate-rise-in font-display text-xs sm:text-sm font-extrabold drop-shadow-md" style={{ background: "linear-gradient(90deg, #FF9933 0%, #FFFFFF 50%, #138808 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Happy 80th Independence Day</span>}
          </div>
        </Link>

        <nav className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => <Link key={item.label} to={item.to} className="rounded-full px-4 py-2.5 text-[0.95rem] font-medium text-ink/80 transition-all duration-500 hover:bg-white/45 hover:text-ink hover:backdrop-blur-xl lg:px-5">{item.label}</Link>)}
        </nav>
