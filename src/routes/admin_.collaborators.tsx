    ...
    <SiteLayout dark>
      <section className="mx-auto max-w-[1400px] px-6 pb-24 lg:px-12">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Only products authorized by the store owner are included in this dashboard.
        </p>
          ...
          <>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <Metric
                ...
                hint="Authorized by owner"
              />
            </div>

            <div className="mt-8 grid gap-4 lg:grid-cols-2">
              {data.links.map((link) => (
                <article key={link.id} className="glass rounded-4xl p-6">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h2 className="font-display text-lg font-extrabold text-ink">{link.name}</h2>
                      {link.url ? (
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <Metric icon={Users} label="Visitors" value={String(link.visitors)} hint="" />
                    <Metric icon={BarChart3} label="Views" value={String(link.page_views)} hint="" />
                    <Metric icon={BarChart3} label="Sales" value={String(link.sales)} hint="" />
                    <Metric
                      icon={BarChart3}
                      label="Revenue"
                      value={`₹${Math.round(link.revenue).toLocaleString("en-IN")}`}
                      hint=""
                    />
                  </div>
                </article>
