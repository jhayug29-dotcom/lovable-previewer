...
export const Route = createFileRoute("/read-more")({
  head: () => ({
    meta: [
      { title: "How Editly Store works — Licence, Delivery & Support" },
      {
        name: "description",
        content:
          "Everything about Editly Store: how instant delivery works, what the commercial licence covers, refunds, FAQs and how to reach support.",
      },
      { property: "og:title", content: "How Editly Store works" },
      {
        property: "og:description",
        content: "Instant delivery, commercial licence, refunds and support — explained in one page.",
      },
...
function ReadMore() {
  ...
  const contactRows = [
    ...
    { icon: Mail, label: "Support", value: settings.support_email, href: `mailto:${settings.support_email}` },
    settings.phone
    ...
    settings.whatsapp
      ...
      : null,
    settings.support_hours ? { icon: Clock, label: "Hours", value: settings.support_hours, href: null } : null,
    settings.address ? { icon: MapPin, label: "Address", value: settings.address, href: null } : null,
  ...
  return (
    <SiteLayout>
      <section className="mx-auto max-w-[1100px] px-6 pb-6 pt-10 lg:px-12">
        <Reveal variant="blur">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-violet-deep">Read more</p>
          ...
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-ink/80">
            Editly Store is a small studio catalogue of editing assets we actually use on client work — motion
            packs, panel extensions, film-emulation LUTs and clean sound libraries. Everything is delivered
            instantly, licensed for commercial use and updated for free.
          </p>
        </Reveal>
      </section>
      ...
      <section className="mx-auto mt-16 max-w-[1100px] px-6 lg:px-12">
        <Reveal>
          <h2 className="font-display text-3xl font-extrabold text-ink sm:text-4xl">Questions, answered.</h2>
        </Reveal>
            ...
            <Reveal key={faq.q} delay={i * 90} variant="scale">
              <div className="glass morph-card h-full rounded-4xl p-7">
                <h3 className="flex items-start gap-2 font-display text-lg font-extrabold text-ink">
                  <HelpCircle className="mt-0.5 size-5 shrink-0 text-violet-deep" strokeWidth={1.8} />
                  {faq.q}
                </h3>
        ...
        <Reveal variant="blur">
          <div className="glass rounded-4xl p-9">
            ...
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              {contactRows.map((row) => (
                <div key={row.label} className="glass-soft flex items-start gap-3 rounded-3xl px-5 py-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-deep">{row.label}</p>
                    {row.href ? (
                      <a
                        href={row.href}
