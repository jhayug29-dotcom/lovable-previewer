...
export const createCollaboratorLink=createServerFn({method:"POST"}).validator(d=>token.extend({name:z.string().trim().min(2).max(80),email:z.string().trim().email()}).parse(d)).handler(async({data})=>(await import("./collaborator.server")).createCollaboratorLink(data.accessToken,data.name,data.email));
